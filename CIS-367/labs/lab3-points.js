...
window.onload = function init() {
	...
	document.getElementById('numpoints_slider').onchange = function(event) {
		...
		if (newParticles < numParticles) {
			...
			numParticles = newParticles; // set the new max	
		}
	...
	document.getElementById('size_slider').onchange = function(event) {
		pointSize = parseInt(event.target.value);
	...
	normalized_width = initialPointSize / gl.canvas.width;
	normalized_height = initialPointSize / gl.canvas.height;
...
var checkCollisions = function() {
	...
	for (var i = 0; i < numParticles; i++) {
		...
		if (particles[i].position[1] > (1.0 - normalized_height)) {
			particles[i].velocity[1] *= -1.0; // reverse x direction
		...
		// bottom wall
		if (particles[i].position[1] < (-1.0 + normalized_width)) {
